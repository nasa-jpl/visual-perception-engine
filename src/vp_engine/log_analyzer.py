...
class LogAnalyzer:
    ...
    def time_diff(df, msg1, proc1, msg2, proc2, head = ""):
        ...
        if diff > 100:
            print(
                f"Warning: time difference between {proc1}.{msg1} and {proc2}.{msg2} for image {df2['image_id'].values[0]} is too high: {int((df2['timestamp'].values[0] - df1['timestamp'].values[0])) * 1e-6} ms"
            )
