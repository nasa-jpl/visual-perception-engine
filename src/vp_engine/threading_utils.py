...
class QueueReceiverThread(StoppableThread):
    def __init__(self, queue, output_list, n_inputs, head_name, logger, timeout: int = 10, store_output: bool = False, *args, **kwargs):
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                last_received_t = perf_counter()
                output = output if self.store_output else None
