...
from utils.colormaps import VOC2012_COLORMAP
...
class SemSegPostprocessingWithVisualizationVOC2012(AbstractPostprocessing):
    """Transform tensor (float) to uint8 tensor, without any scaling."""
    ...
    def __init__(
        ...
        self._device = None
        self.colormap = torch.from_numpy(np.array(VOC2012_COLORMAP, dtype=np.uint8))
    ...
    def __call__(self, data: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        # interpolate to original size and choose one class per pixel
        output = F.interpolate(x, size=(self.output_height, self.output_width), mode="bilinear", align_corners=False)
        output = F.softmax(output, dim=1).argmax(dim=1, keepdim=True).squeeze(0).permute(1, 2, 0).long()
        
        if not self._device:
            self._device = output.device
            self.colormap = self.colormap.to(self._device)
        
        colored_output = self.colormap[output.squeeze() + 1]
        return {POSTPROCESSING_OUTPUT: colored_output.to(torch.uint8)}
    ...
    @property
    def output_signature(self) -> dict[str, tuple]:
        return {POSTPROCESSING_OUTPUT: (self.output_height, self.output_width, self.colormap.shape[-1])}
