...
def inference(
    ...
    stream_identifier = torch.cuda.current_stream().cuda_stream
    eval_start_time = time.perf_counter()
    input_img = torch.tensor(input)
    img_cuda = input_img.to(device=torch.device("cuda")) # because its an image it should be torch.uint8
...
def run(
    ...
    fm_in_signature = model.model.input_signature if hasattr(model, "model") else model.fm_model.input_signature
    preprocessing_func_init = preprocessing_func(fm_signature=fm_in_signature, fm_type=precision, canonical_height=orig_h, canonical_width=orig_w)
    mh_out_signature = model.model.output_signature if hasattr(model, "model") else model.mh_model.output_signature
    postprocessing_fn = ResizeAndToCV2Image(mh_signature=mh_out_signature, mh_type=precision, canonical_height=orig_h, canonical_width=orig_w)
